...
{
  INPUT.FILE1 = '~/10x/data/mut/filtered_feature_bc_matrix.h5'
  INPUT.FILE2 = '~/10x/data/mut/atac_fragments.tsv.gz'
  
  OUTPUT.FILE1 = 'ANL001a.mut.data.processed.rds'
}
...
{ ### QC and filtering ###
  ...
  sobj.filt = subset(
    x = sobj, 
    subset = nCount_RNA < 5e3 & nCount_RNA > 50 & 
      nCount_ATAC < 8e4 & nCount_ATAC > 500 & 
      percent.mt < 15
  )
...
saveRDS(sobj.filt, file = OUTPUT.FILE1)
